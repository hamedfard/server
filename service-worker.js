...
sharedLib.IndexedDBAdapter.prototype._withDatabase = function(action) {
    ...
    openRequest.onupgradeneeded = function(event) {
        ...
        for (var key in self.__database.OBJECT_STORES)
...
;
var APPBOY_PUSH_TAG = "static-appboy-push-tag";
var showNotification = function(payload) {
    ...
    if (shouldFetchTriggers) {
        console.log("Appboy: service worker 1.6.4 found trigger fetch key in push payload.");
...
self.addEventListener("push", function(event) {
    console.log("Appboy: service worker 1.6.4 received push");
    if (event.data != null && event.data.json != null)
        ...
        var promise = new Promise(function(resolve, reject) {
            ...
            (new sharedLib.IndexedDBAdapter(db,{
                ...
                fetch(dbEntry.baseUrl + "/server/web_push/", {
                    method: "POST",
                    headers: {
...
self.addEventListener("notificationclick", function(event) {
    ...
    if (Notification.prototype.hasOwnProperty("data")) {
        ...
        dbAdapter.getLastItem(db.OBJECT_STORES.DATA, function(key, dbEntry) {
            ...
            data.sdk_version = "1.6.4";
            fetch(dbEntry.baseUrl + "/server/data/", {
                method: "POST",
                headers: {
                    "Content-type": "application/json"
                },
